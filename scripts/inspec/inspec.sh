...
SCAN_ID=`archerysec-cli -s ${ARCHERYSEC_HOST} -u ${ARCHERY_USER} -p ${ARCHERY_PASS} --upload --file_type=JSON --file=${WORKSPACE}/reports/inspec_scripts/tomcatproduction.json --TARGET=''$COMMIT_ID'' --scanner=inspec --project_id=''$PROJECT_ID'' | tail -n1 | jq -r ".scan_id"`
                     
echo $SCAN_ID

job_status=`python ${WORKSPACE}/scripts/inspec/inspec_test.py --failed 100 -f ${WORKSPACE}/reports/inspec_scripts/tomcatproduction.json | grep SUCCESS`
