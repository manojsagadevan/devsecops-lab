...
sleep 20
python ${WORKSPACE}/scripts/vuln_check_rule.py --scanner=dependencycheck --scan_id=$SCAN_ID --username=${ARCHERY_USER} --password=${ARCHERY_PASS} --host=${ARCHERYSEC_HOST} --high=${DCHIGH} --medium=${DCMEDIUM}
# Provide the rule on High and Medium Vulnerabilities
job_status=`python ${WORKSPACE}/scripts/vuln_check_rule.py --scanner=dependencycheck --scan_id=$SCAN_ID --username=${ARCHERY_USER} --password=${ARCHERY_PASS} --host=${ARCHERYSEC_HOST} --high=${DCHIGH} --medium=${DCMEDIUM} | grep SUCCESS`
